...
def index2():
    ...
    # 讀取三大法人進出資料
    cursor.execute("SELECT * FROM stock.stock_all_data where TABLE_TYPE='三大法人進出-正規化'")
    stock_daily_3_db_normalization = cursor.fetchall()
    stock_daily_3_db_normalization_list = []
    for row in stock_daily_3_db_normalization :
        d = collections.OrderedDict()
        d['COLUMN_NAMES']  = row[0] #name
        d['DATA_TYPE']   = row[1] #lname
        d['IS_NULLABLE']      = row[2] #email
        ...
        d['DESCS']      = row[4] #email
        d['TABLE_TYPE']      = row[5] #email
        stock_daily_3_db_normalization_list.append(d)
    
    # 讀取融資融券總表-正規化資料
    cursor.execute("SELECT * FROM stock.stock_all_data where TABLE_TYPE='融資融券總表-正規化'")
    stock_daily_MT_All = cursor.fetchall()
    stock_daily_MT_All_list = []
    for row in stock_daily_MT_All :
        d = collections.OrderedDict()
        d['COLUMN_NAMES']  = row[0] #name
        d['DATA_TYPE']   = row[1] #lname
        d['IS_NULLABLE']      = row[2] #email
        ...
        d['DESCS']      = row[4] #email
        d['TABLE_TYPE']      = row[5] #email
        stock_daily_MT_All_list.append(d)
    
    # 讀取融資總表-正規化資料
    cursor.execute("SELECT * FROM stock.stock_all_data where TABLE_TYPE='融資總表-正規化'")
    stock_daily_MT_Financing = cursor.fetchall()
    stock_daily_MT_Financing_all = []
    for row in stock_daily_MT_Financing :
        d = collections.OrderedDict()
        d['COLUMN_NAMES']  = row[0] #name
        d['DATA_TYPE']   = row[1] #lname
        d['IS_NULLABLE']      = row[2] #email
        ...
        d['DESCS']      = row[4] #email
        d['TABLE_TYPE']      = row[5] #email
        stock_daily_MT_Financing_all.append(d)

    # 讀取融券總表-正規化資料
    cursor.execute("SELECT * FROM stock.stock_all_data where TABLE_TYPE='融券總表-正規化'")
    stock_daily_MT_Securities = cursor.fetchall()
    stock_daily_MT_Securities_all = []
    for row in stock_daily_MT_Securities :
        d = collections.OrderedDict()
        d['COLUMN_NAMES']  = row[0] #name
        d['DATA_TYPE']   = row[1] #lname
        d['IS_NULLABLE']      = row[2] #email
        ...
        d['DESCS']      = row[4] #email
        d['TABLE_TYPE']      = row[5] #email
        stock_daily_MT_Securities_all.append(d)
